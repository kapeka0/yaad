...
class FakeRepository implements GrepRepository {
  ...
  async resolveOccurrences(
    ...
    return {
      occurrenceIds: Uint32Array.from(this.occurrences.map((row) => row.jsId)),
      totalResults: 3,
      totalHosts: 3,
      unassignedResults: 1,
      unassignedHosts: 1,
...
test("full search is asynchronous, retries storage, returns exact snapshot totals and cursor pages", async () => {
  ...
  assert.equal(first.unassignedResults, 1);
  assert.equal(first.unassignedHosts, 1);
  ...
  assert.equal(next.body.results[0].programName, null);
  assert.equal(next.body.results[0].attribution, "unassigned");
  ...
  assert.deepEqual(repository.pageIdCalls, [[3, 8], [12]]);
});
